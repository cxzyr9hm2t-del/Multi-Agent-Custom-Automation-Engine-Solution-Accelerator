...
sys.modules['common.models.messages'] = Mock(TeamConfiguration=Mock())
...
sys.modules['models.messages'] = Mock(
...
AgentMessageStreaming = sys.modules['models.messages'].AgentMessageStreaming
AgentToolCall = sys.modules['models.messages'].AgentToolCall
AgentToolMessage = sys.modules['models.messages'].AgentToolMessage
WebsocketMessageType = sys.modules['models.messages'].WebsocketMessageType
...
class TestExtractToolCallsFromContents:
    ...
    def test_extract_tool_calls_with_function_calls(self):
        ...
        with patch('backend.callbacks.response_handlers.AgentToolCall') as mock_agent_tool_call:
...
class TestAgentResponseCallback:
    ...
    def test_agent_response_callback_with_chat_message(self, mock_now, mock_create_task):
        ...
        with patch('backend.callbacks.response_handlers.AgentMessage') as mock_agent_message:
            mock_agent_msg = Mock()
    ...
    def test_agent_response_callback_exception_handling(self, mock_create_task, mock_logger):
        ...
        with patch('backend.callbacks.response_handlers.AgentMessage'):
            agent_response_callback("agent_123", mock_message, user_id="user_456")
...
class TestStreamingAgentResponseCallback:
    ...
    async def test_streaming_callback_with_tool_calls(self):
        ...
        with patch('backend.callbacks.response_handlers._extract_tool_calls_from_contents') as mock_extract:
            mock_tool_call = Mock()
            mock_extract.return_value = [mock_tool_call]

            with patch('backend.callbacks.response_handlers.AgentToolMessage') as mock_tool_message:
                mock_tool_msg = Mock()
    ...
    async def test_streaming_callback_exception_handling(self):
        ...
        with patch('backend.callbacks.response_handlers.logger') as mock_logger:
            with patch('backend.callbacks.response_handlers.AgentMessageStreaming'):
                await streaming_agent_response_callback("agent_123", mock_update, False, user_id="user_456")
